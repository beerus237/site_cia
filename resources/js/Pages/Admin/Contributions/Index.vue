  ...
  >
    <div class="space-y-6">
      ...
      <div class="surface-card flex flex-wrap items-center gap-3 p-4">
        ...
        <select v-model="filters.campagne_id" class="field-input w-auto py-2">
          <option value="">Toutes les campagnes</option>
          <option v-for="campagne in campagnes" :key="campagne.id" :value="campagne.id">{{ campagne.titre }}</option>
        </select>
...
const filters = ref({
  ...
  campagne_id: props.filters?.campagne_id || '',
})
...
const filteredContributions = computed(() =>
  props.contributions.data.filter((contribution) => {
    ...
    if (filters.value.campagne_id && String(contribution.campagne?.id) !== String(filters.value.campagne_id)) return false
    return true
  })
)
...
function resetFilters() {
  filters.value = { type: '', statut: '', campagne_id: '' }
}
